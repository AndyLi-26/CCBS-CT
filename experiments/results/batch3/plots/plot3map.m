...
algo=["-0-0-0-0","-0-0-ds-0","-0-0-0-icp","-0-0-ds-icp"];
algo_name=["vanillia","ds","icp","icp+ds"];
all_data=cell(3,2,4);
a_size=[0.5,4.5];
for m=1:3
    for alg=1:4
        T = readtable(strcat(maps(m),algo(alg),".csv"));
        T=T{:,:};
        for a=1:2
            temp=T(T(:,2)==a_size(a),:);
            all_data{m,a,alg}=temp; 
        end
    end
end
%plot succ rate
...
for m=1:3
    for a=1:2
        subplot(2,3,(a-1)*3+m); hold on; title(sprintf("%s agent size=%f",maps(m),a_size(a)));
        for algo=1:4
            data=all_data{m,a,algo};
            x=unique(data(:,1));
