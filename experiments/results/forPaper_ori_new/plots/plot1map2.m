...
a_num=[37,38,22];
algo=["-0-0-ds-0","-0-ct_abs-ds-icp"];
algo_name=["Node Expansion: I-CCBS","Node Expansion: I-CCBS-MCT"];
a_size=[0.5];
for m=1:3
...
%     xticks(x);
    ...
    comparsion=[1,2];
            ...
            title(sprintf("%s\nbetter: %d, same: %d, worse: %d",map(m),sum(check2 & ~check3), sum(eqcheck & ~check3), sum(check1 & ~check3)));
