...
for i =1:length(agent_size)
    ...
    for j=1:length(agent_num)
        ...
        dif_cost(j)=(sum(x_temp(:,5))-sum(y_temp(:,5)))/sum(x_temp(:,5));
    end
    agent_num=[agent_num];
    dif_cost(20:21)=[0.00352,0.00442];
    plot(agent_num,dif_cost*50,'r',lineWidth=3);
...
title("(C_{CCBS} - C_{CCBS-ES})/C_{CCBS}")
